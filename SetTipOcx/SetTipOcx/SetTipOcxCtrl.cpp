...
BEGIN_DISPATCH_MAP(CSetTipOcxCtrl, COleControl)
	...
	DISP_FUNCTION_ID(CSetTipOcxCtrl, "add", dispidadd, add, VT_I4, VTS_I4 VTS_I4)
END_DISPATCH_MAP()
...
void CSetTipOcxCtrl::show(LPCTSTR strText)
{
	...
	::MessageBox(this->m_hWnd, strText, NULL, MB_OK);
	return;
}

LONG CSetTipOcxCtrl::add(LONG a1, LONG a2)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	// TODO: 在此处添加分派处理程序代码
	//::MessageBox(this->m_hWnd, "add", NULL, MB_OK);
	return a1 + a2;
}
